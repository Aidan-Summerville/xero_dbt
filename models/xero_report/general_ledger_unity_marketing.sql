...
with journals as (
    ...
    select *
    from "FIVETRAN_INPUT"."UNITY_MARKETING"."JOURNAL"

), journal_lines as (

    select *
    from "FIVETRAN_INPUT"."UNITY_MARKETING"."JOURNAL_LINE"

), accounts as (

    select *
    from "FIVETRAN_INPUT"."UNITY_MARKETING"."ACCOUNT"

), joined as (

    select 
        'Unity Marketing Ltd' agency_name,
        journals.journal_id,
